...
## 十、第二輪：放大、角度、季節色（使用者 2026-08-26，第一輪上線後）

### 10.1 判準整個換掉：可以壓格子，不可以壓導航

第一輪把「疊印不可以切到活動格子裡的任何東西」原封帶過來，於是章被壓到很小
（`.e2` 14px），而且 `top` 一路降到 25px 貼著頁面上緣。

**使用者裁定推翻那條判準**：

> 章壓到第二、三格沒關係，那個「壓過去」正是「這個月結束了」的感覺，
> 而且真的入境章本來就蓋在已經有東西的頁面上。月份章只在三格全蓋滿才出現，
> 所以底下一定都完成了。唯一要守的：不要壓到左上角的月份數字和月名，
> 那是導航不是內容。

新判準：**可以壓 `.slot` 與格子裡的內容；`.mnum` 與 `.mzh` 的重疊面積必須是 0。**

**被取代的原文保留**（同 2026-08-25 裁定 2 的做法）：舊判準當時是對的
——那時章是一行文字的疊印，壓到 `.cat` 只是把標籤切掉一半，沒有任何語意。
現在章大到足以構成一個「蓋章」的動作，壓過去本身變成訊息。

**這條裁定讓 `pointer-events:none` 從裝飾變成載重的**：章現在蓋在可點的格子上，
少了它，蓋滿的月份會有兩格點不開，而且不會有任何東西報錯。要加守門。

### 10.2 角度由「月份 + uuid」決定，範圍 -15° 到 +8°

跟活動章同一套機制（由 id 算、不是隨機，重整不會跳）。種子多帶一個月份，
讓同一本護照的十一枚章各有各的角度 —— 整本同一個角度看起來像印刷，
不像一枚一枚蓋上去的。

不對稱是刻意的：逆時針多一點、順時針少一點，跟大多數人右手蓋章的手腕角度一致。

**不要順手把 `stampHTML` 的角度公式也改成 `hash32`** —— 那會讓每一枚已經蓋出去的
活動章換角度。

**量測的注意事項**：正角度會把章往另一邊倒，左下角更靠近 `.mzh`。
`.mzh` 那條限制的最壞情況很可能是 `+8°` × 最長城市，不是 `-15°`。

### 10.3 十一色季節漸變，首尾同色

九月暖橘 → 冬天深藍 → 春天青綠 → 七月回到暖橘。
**首尾同色，翻完一本有回到原點的感覺，那正好是學年的形狀。**

### 10.4 這跟三色守門的衝突怎麼處理

使用者的指示：

> 不要放寬那條檢查 —— 它今天才擋下過真實錯誤，放寬等於從此失效。
> 改成把 `.estamp` 列為明確的例外：允許自己的一組顏色，其他地方維持三色。
> **例外清單要寫死在檢查裡，不是用萬用字元。**

設計：色盤住在 `index.html` 一個用哨兵註解框起來的區塊，`check.sh` 加三條守門：

1. **哨兵各自只准出現一次** —— 抽錯範圍的守門比沒有守門更糟（README 第 10 項）
2. **區塊內的色碼必須剛好等於寫死的清單**（集合相等，不是包含）——
   讓「偷偷加第十二色」不可能，而不只是「不鼓勵」
3. **這些色碼不准出現在區塊外面** —— 季節色是入境章專用的，
   不是「解禁了十色可以到處用」

既有那條「只有三個色碼」的掃描把色盤區塊**排除在外**，不是加白名單：
白名單會讓那十色在任何地方都合法，那就是使用者說的「放寬」。

一個月只有一個色碼（`--ink`），底色若需要則用
`color-mix(in srgb, var(--ink) 14%, transparent)` 導出 —— 例外清單維持十筆，
不會因為要底色就變二十筆。

### 10.5 ~~橫跨三格~~（2026-08-26 試過，否決，維持右上角）

**這個方向在紙上成立、量測全過、實際看起來不好。** 記在這裡是因為下一個人會問
同樣的問題 —— 讓他知道我們試過，而且知道試到什麼程度。

使用者當時的判斷（原話）：

> 它縮在右上角，跟三格是平行的關係，只碰到第三格一角，中間兩格完全沒被影響。
> 讀起來像「這個月的標籤」，不是「這個月被封起來了」。要的是：章橫跨蓋在三格中間。

規格是：寬度約三格總寬的 55-70%、水平置中、垂直在三格中段（活動章那一排的高度）、
角度縮到 -6°～+4°、三格內容從筆畫之間透出來。

**做了三版原型（不進 repo，全部在 scratch 用注入的 CSS 跑）。桌機 1280px、dpr 2、
最長城市 SALT LAKE CITY：**

| 佔三格寬 | 角度 | 章寬 | `.e2` 字級 | 碰到幾格 | 超出頁面 | 壓到月名 |
|---|---|---|---|---|---|---|
| 55% | -6° | 500.2 | 35.1px | **3** | 否 | 0 |
| 65% | -2° | 578.2 | 41.8px | **3** | 否 | 0 |
| 75% | +4° | 675.1 | 48.0px | **3** | 否 | 0 |

三版全部達成「碰到三格、不超出頁面、不壓月名」。**判準全過。**

**否決的理由是看起來不對，不是量測不過。** 兩個具體的問題：

1. **框太細。** 章放大到 500-675px，但邊框仍是 3-4px 的 double，讀起來像一個
   空框裡放大字，不像蓋章。真的海關章框線會隨尺寸一起加重。
2. **墨的質感消失。** `filter:url(#bt-ink)` 的擾動是固定尺度的，章一大就相對變細，
   邊緣看起來像向量圖不像油墨。

**手機根本不成立。** 390px 三格是**直向堆疊**的，沒有「三格中間」這個位置。
直接照搬的結果是章整個落在中間那格上、**跟該格的活動章疊在同一個位置**，
城市名折成兩行，量測 `碰到幾格 = 1`。另做了一版跨在第一、二格交界上
（單欄寬的 72/82/92%、`white-space:nowrap`），碰到兩格、不撞活動章 ——
但那已經是另一個設計，不是同一個概念在小螢幕上的版本。

**連帶會失效的兩條**（如果哪天再走這個方向，這兩條要一起處理）：

- `.estamp::before` 那條擦線規則的前提消失（章不再碰到 `.mhead` 的線），
  該移除不是留著（README 第 11 項）
- `pointer-events:none` 從「兩格點不開」升級成「**整列點不開**」

**維持的版本**：右上角錨定、26px 那組字級、角度 -15°～+8°、B 版無底色、
`::before` 擦線。正式站 `ea725ab`。
