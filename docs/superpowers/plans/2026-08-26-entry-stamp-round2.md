...
使用者 2026-08-26 給了方向，並且指出這不是二選一：

> 我傾向 B（只有墨線），但如果 B 讓手機的日期又被線穿過去，
> 那就不是 A 或 B 的選擇，是「**B 加上把那條線在章的範圍內斷開**」。

所以要做**第三版**：

| C | B ＋ 把 `.mhead` 那條線在章的範圍內斷開 | 線在章底下讓開，其餘完全是 B |

### C 怎麼實作（要量，不要猜）

`.estamp` 因為 `transform` 會自成一個 stacking context，
所以它的子元素**再怎麼壓也壓不到 `.estamp` 自己下面**——
`z-index:-1` 的子元素會落在「章的墨線之下、但仍然在 `.page` 內容之上」，
正好是我們要的那一層。

```css
  .estamp::before{
    content:"";position:absolute;z-index:-1;
    left:0;right:0;top:<量出來>px;height:<線寬+2>px;
    background:var(--paper);
    transform:rotate(calc(-1 * var(--rot)));   /* 反轉回水平 */
    transform-origin:center;
  }
```

`--rot` 跟 `transform` 用同一個 inline 值（Task 1 已經把角度做成 inline style，
順手多帶一個 custom property 即可）。

**三個要量的東西**：

1. `top` 是「線的 y 減章的未旋轉 top」，**兩個視寬各量一次**，不要用同一個值
2. 橫向範圍 `left:0;right:0` 是章的元素寬度，比旋轉後的外接框窄——
   **截圖確認斷開的缺口看起來像「章壓在線上」，不是「線莫名其妙斷了一截」**。
   缺口太短就用負的 `left`／`right` 往外推，但**推多少要量，不要抓 40px 這種數字**
3. 反轉之後 `::before` 是水平的：確認它跟線平行，不是斜的

- [ ] **Step 1: 三版各截 1280 與 390 兩張**，用 Task 2 選定的尺寸與最壞城市。
  C 版另外截一張把角度設成 `+8°` 的（正角度下缺口的位置最容易跑掉）
- [ ] **Step 2: 停下來等使用者裁定**
