...
--
-- ⚠ 2026-08-31：這份測試描述的是拆表之前的世界，**現在跑它的結果沒有意義**。
--   它測「B 看得到 A 的 passports」而進度牆已經改讀 profiles；
--   它測的註冊路徑在階段 5 會整個消失（邀請碼從註冊移到角色升級）。
--   規格 §3-5 要求在階段 5 改寫這一份，並且做反向驗證
--   （拿掉邀請碼檢查，確認測試真的會 FAIL）。**在那之前不要拿它的綠燈當保證。**
--
